...
<c:if test="${not empty sessionScope.admin}">
	<div class="container">
		...
		<div id="editlessonBox">
			<form class="layui-form layui-form-pane" action="" lay-filter="form2">
				<div class="layui-form-item">
					<label class="layui-form-label">课程</label>
					<div class="layui-input-block">
						<select name="lesson" lay-verify="required" id="shezhil">
							<option value="">请选择该教师教授课程</option>
						</select>
					</div>
				</div>
				<div class="layui-form-item" style="text-align: center;">
...
<a class="layui-btn  layui-btn-xs" lay-event="shezhi">设置课程</a>
<a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="del">删除</a>
	...
	<script type="text/javascript">
		layui.use('table', function () {
			...
			table.render({
				elem: '#test'
				, url: 'teacherController/list' //数据接口
				, method: 'post'
				...
				, cols: [[
					...
					, {
						field: 'lesson.lessonName', title: '所授课程', templet: function (res) {
							if (res.lesson != null) {
					...
					case 'delData':
						...
						if (arr.length != 0) {
							layer.confirm('真的要删除么', function (index) {
								...
								console.log(arr);
								$.ajax({
									url: "teacherController/deleteAll",
									type: "post",
									data: { "arrs[]": arr },
									dataType: "json",
			...
			table.on('tool(test)', function (obj) {
				...
				} else if (obj.event === 'shezhi') {
					lesson_edit(data.user.id);
				}
			});
		});
		...
		function lesson_edit(id) {
			...
			layui.use('form', function () {
				...
				$.ajax({
					...
					success: function (data) {
						...
						$.each(data.data, function (i, item) {
							...
							$('#shezhil').append(new Option(item.lessonName, item.id));// 下拉菜单里添加元素
						});
				...
				form.on('submit(form2)', function (data) {
					...
					$.ajax({
						type: "POST",
						url: "teacherController/update_lesson",
						data: { "user.id": userid, "lesson.id": formdata.lesson },
						dataType: "JSON",
						success: function (data) {
							if (data.result == "yes") {
								layer.closeAll();
